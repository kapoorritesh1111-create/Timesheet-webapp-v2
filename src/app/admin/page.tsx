...
function AdminInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const list = ((((mgrs as any) ?? []) as ManagerRow[]) || []) as ManagerRow[];
      setManagers(list);
  ...
  async function sendInvite(e: React.FormEvent) {
    ...
    try {
      ...
      const body = {
        ...
        project_ids: [], // keep empty here; assign on Users drawer if needed
      };
      ...
      // ✅ Step 19: take admin straight to Users + auto-open drawer
      if (json?.userId) {
        router.push(`/admin/users?user=${encodeURIComponent(String(json.userId))}`);
      }
    } catch (err: any) {
        ...
        <form onSubmit={sendInvite} style={{ marginTop: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
            ...
            <div>
              ...
              {inviteRole === "manager" ? (
                <div className="muted" style={{ marginTop: 6, fontSize: 12 }}>
                  Managers don’t use hourly rate for payroll.
                </div>
              ) : null}
            </div>
          </div>
